...
public final class GridSampleModel extends ComponentSampleModel
{
	
	/********added Andrew Reid 211008*****************/
	 
	public boolean[][][] mask;
	public boolean apply_mask = true;
	public int z;
     ...
     */
    public GridSampleModel(int dataType, int w, int h, int numBands) {
	super(dataType, w, h, 1, w,
			GridSampleModel.createIndicesArray(numBands),
			GridSampleModel.createOffsetArray(numBands));
    }
     ...
     *         one of the supported data types
     */
    public GridSampleModel(int dataType,
			     int w, int h,
			     int scanlineStride,
			     int bankIndices[],
                             int bandOffsets[]) {
    ...
    protected boolean is_masked(int x, int y){
		if (!apply_mask || mask == null) return false;
		return mask[x][y][z];
	} 
	...
	public SampleModel createCompatibleSampleModel(int w, int h) {
        ...
        SampleModel sampleModel = 
            new GridSampleModel(dataType, w, h, w, bankIndices, bandOffs);
	return sampleModel;
    }

    /**
     * Since this is hidden in ComponentSampleModel... 
     */
   int []orderBands(int orig[], int step) {
	...
	for (int i=0; i<bands.length; i++) {
        ...
        return new GridSampleModel(this.dataType, width, height,
				     this.scanlineStride,
	...
	case DataBuffer.TYPE_USHORT:
	    ...
	    for (int i=0; i<numDataElems; i++) {
	    	if (is_masked(x, y)){
	    		//InterfaceSession.log("" + x + "," + y + ":masked");
	    		sdata[i] = 0;
	    	}else
	...
	public float getSampleFloat(int x, int y, int b, DataBuffer data) {
        ...
        if (is_masked(x, y)) return Float.NaN;
	...
	public double getSampleDouble(int x, int y, int b, DataBuffer data) {
        ...
        if (is_masked(x, y)) return Double.NaN;
	...
	int bank = bankIndices[b];
        ...
        for (int i = 0; i < h; i++) {
           ...
           for (int j = 0; j < w; j++) {
        	  if (is_masked(i, j))
        		  samples[srcOffset++] = Integer.MAX_VALUE;
        	  else
