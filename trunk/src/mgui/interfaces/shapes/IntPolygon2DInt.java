...
public class IntPolygon2DInt extends Polygon2DInt {

	public ArrayList<MguiInteger> integerNodes;
	...
	private void init(){
		...
		attributes.add(new Attribute("LabelTrue", new MguiBoolean(true)));
		attributes.add(new Attribute("LabelSize", new MguiInteger(10)));
		
		/**@TODO add shape & colour tables to determine symbol display based
		 * upon node values */
	...
	public void resetNodes(){
		integerNodes = new ArrayList<MguiInteger>();
		if (this.thisShape != null){
			for (int i = 0; i < ((Polygon2D)thisShape).vertices.size(); i++)
				integerNodes.add(new MguiInteger(0));
		}
	}
	...
	public void setNodeVal(int node, int val){
		if (node > -1 && node < integerNodes.size())
			integerNodes.get(node).setValue(val);
	}
	
	public int getNodeBool(int node){
		if (node > -1 && node < integerNodes.size())
			return integerNodes.get(node).getInt();
		return 0;
	}
	...
	public void drawShape2D(Graphics2D g, DrawingEngine d){
		...
		if (((MguiBoolean)attributes.getValue("HighlightTrue")).getTrue())
			...
			for (int i = 0; i < this.getPolygon().vertices.size(); i++)
				if ((integerNodes.get(i)).getInt() != 0){
					...
					//draw label
					if (((MguiBoolean)attributes.getValue("LabelTrue")).getTrue()){
						Map2D thisMap = d.getMap();
						Point thisPt = thisMap.getScreenPoint(getPolygon().vertices.get(i));
						g.setFont(new Font("Arial",
										   Font.PLAIN,
								           ((MguiInteger)attributes.getValue("LabelSize")).getInt()));
						g.drawString((integerNodes.get(i)).toString("#0"),
									 thisPt.x + 3, thisPt.y - 3);
						}
					}
	...
	public String toString(){
		return "IntPolygon2DInt [" + String.valueOf(ID) + "]"; 
	}
