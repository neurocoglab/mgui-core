/*
* Copyright (C) 2014 Andrew Reid and the ModelGUI Project <http://mgui.wikidot.com>
* 
* This file is part of ModelGUI[core] (mgui-core).
* 
* ModelGUI[core] is free software: you can redistribute it and/or modify
* it under the terms of the GNU General Public License as published by
* the Free Software Foundation, either version 3 of the License, or
* (at your option) any later version.
* 
* ModelGUI[core] is distributed in the hope that it will be useful,
* but WITHOUT ANY WARRANTY; without even the implied warranty of
* MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
* GNU General Public License for more details.
* 
* You should have received a copy of the GNU General Public License
* along with ModelGUI[core]. If not, see <http://www.gnu.org/licenses/>.
*/

package mgui.interfaces.tools.shapes;

import mgui.interfaces.InterfaceOptions;
import mgui.interfaces.InterfaceSession;
import mgui.interfaces.attributes.AttributeList;
import mgui.interfaces.shapes.Mesh3DInt;
import mgui.interfaces.shapes.SectionSet3DInt;
import mgui.interfaces.shapes.ShapeSet3DInt;

public class ToolCreateMeshFromPolylinesOptions extends InterfaceOptions {

	public SectionSet3DInt source_set;
	public String name = "mesh from polygons";
	public boolean resample = false;
	public boolean redirect = true;
	public float min_seg_length = 5;
	public boolean use_2D = true, use_3D = true;
	public AttributeList attributes;
	public ShapeSet3DInt target_set;
	public double weight_threshold = 0.8;
	
	/*
	public ToolCreateMeshFromPolylinesOptions(){
		this(null);
	}
	*/
	
	public ToolCreateMeshFromPolylinesOptions(){
		//set default attributes
		Mesh3DInt mesh = new Mesh3DInt();
		attributes = mesh.getAttributes();
		//if (panel != null)
		target_set = InterfaceSession.getDisplayPanel().getCurrentShapeSet();
		//target_set = panel.getCurrentModel().getModelSet();
	}
	
}