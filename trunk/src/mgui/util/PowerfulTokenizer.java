...
public class PowerfulTokenizer implements Enumeration
{
	...
	public String nextToken()
	{
						...
						&& (!sToken.trim().endsWith("\"\"")))) )
		{
			...
			{
				String sNextToken = oTokenizer.nextToken();
				//InterfaceSession.log("next token = " + sNextToken 
				//							+ ", token = " + sToken);
		...
		//InterfaceSession.log("idx = " + iTokenNo + ", token = " + sToken);
		return sToken;
	}
	...
	public int countTokens()
	{
		...
		InterfaceSession.log("original tokens = " + iTokens);
		...
		for (int i=1; i<aiIndex.length; i++)
		{
			...
			aiIndex[i] = iIndex;
			//InterfaceSession.log("aiIndex[" + i + "] = " + iIndex);
	...
	private boolean isWithinQuotes(int k)
	{
		...
		//InterfaceSession.log("quote = " + iStartQuote);
	...
	public static void main(String[] args)
	{
		...
		InterfaceSession.log("no of tokens = " + iTokens);

		/*for (int i=0; i<iTokens; i++)
		{
			InterfaceSession.log("token" + i + " = " + oMT.nextToken());
		}
